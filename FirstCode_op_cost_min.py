# -*- coding: utf-8 -*-
"""
Created on Fri Jan 31 12:18:17 2020
...
pv_cap = 20 #installed photovoltaic installed capacity in Kilowatts
cost=0.25   #Electricty price in Euro/KWh
fit=0.10    #Feedin Tariff in Euro/KWh
SOC_0 = 0 #Assumed initialstate of Charge
...
model += pulp.lpSum( 
        [(cost * El_grid[k] - fit * PV_feed[k]) for k in Data.index]
        )
...
for k in range (1,8758):
    ...
    if  PV_generation[k+1]['PV[kW]'] >= (EL_load[k+1]["el_load_house[kW]"] + E_boiler[k+1] + E_hp[k+1]):
            model += SOC[k+1] == SOC[k]*eff_storage - (H_st[k+1])*(dt)  #charging the storage
           
    else:
#        PV_generation[k] <= (EL[k] + E_boiler[k] + E_hp[k]):
            model += SOC[k+1] == SOC[k]*eff_storage - (H_st[k+1])*(-dt) #discharging the storage
   

# The problem data is written to an .lp file
model.writeLP('Operation Cost Minimization problem.lp')
...
### Write Files
file_name = "Operation Cost Minimization problem.csv"
